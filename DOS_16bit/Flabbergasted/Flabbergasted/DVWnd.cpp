...
void CDVWnd::Initialize()
{
	...
	m_bActive = TRUE;
								...
								// Last Out order.
	try {
		m_dvdrawobject.CreateFullScreen(m_iResX,m_iResY,m_iResBitDepth,m_hWnd,BUFFERTYPE_BLIT);
		m_dvinputobject.Create(hInstance, m_hWnd, NULL, DVINPUT_MOUSE | DVINPUT_KEYBOARD);
		m_dvsoundobject.Create(m_hWnd,FREQ_22KHZ);
	}
	catch(DVERR &dve) {
...
/*===========================================================================
	...
	DESCRIPTION:	All the work for graphics and sound happens here.
...
void  CDVWnd::DoFrame()
{
	...
	m_dvinputobject.Refresh(DVINPUT_KEYBOARD | DVINPUT_MOUSE);	
	mx = m_dvinputobject.m_iMouseX;
	my = m_dvinputobject.m_iMouseY;

								// Quit if a button is pressed
	if(	m_dvinputobject.m_iMouseButton[0]){
		m_bActive = FALSE;
	}


	// TODO:  Replace the next few lines of code with your own drawing routines.
	m_dvdrawobject.Clear(0);																  									   
	m_dvdrawobject.Print(DM_SET,0,mx-20,my-10,
		"TODO: Add code to\n"
		"draw stuff here.\n"
		"Frames per second: %.0lf\n"
		"Click the left mouse button to exit\n",m_dFPS);		 



								// VARMINT SAYS:  This line is _EXTREMELY_ important.  
								// It unlocks the DirectDraw screen buffer, which
								// is necessary in order for windows messages to 
								// work.  Failing to do this on Windows95 will 
								// hang the computer.
	m_dvdrawobject.EndDrawing();
	
								// If you want to play music, uncomment this next 
								// line to keep the mixer running.
	// m_dvsoundobject.MixAhead(50);

 								// Swap the visible and drawing surfaces.  (This has
								// no effect in single buffered mode)
	m_dvdrawobject.PageFlip();
