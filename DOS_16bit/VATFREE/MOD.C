		...
		switch(e1) {                            // Process the commands
			...
			case 15:                            // Set Speed.
				...
				if(j <= 32) {
					...
					mod_bytespertick = ((LONG)sample_rate * 60L)/(24L * j);
...
MOD VFAR *LoadMod(CHAR *filename,CHAR *errstring)
{
	...
	j = fseek(input,1080, SEEK_SET);        // Look for signatures so we
																					// can tell of this MOD has 15
																					// Or 31 samples.
	if(j) {
		sprintf(errstring,"Seek Error");
		VFREE(m);
		fclose(input);
		return(NULL);
	}

	j = fread(signature,1,4,input);
	signature[4] = 0;
	if(j != 4) {
		sprintf(errstring,"Error Reading signature");
		VFREE(m);
		fclose(input);
		return(NULL);
	}
	...
	if     (!strcmp(signature,"")) samples = 15;
	else if(!strcmp(signature,"M.K.")) samples = 31;
	else if(!strcmp(signature,"M!K!")) samples = 31;
	else if(!strcmp(signature,"FLT4")) samples = 31;
	else {
		sprintf(errstring,"Unrecognized MOD signature: '%s'",signature);
		VFREE(m);
		fclose(input);
		return(NULL);
	}
	...
	j = fread(m->title,1,20,input);         // read the module title
	if(j != 20) {
		sprintf(errstring,"Error Reading Title");
		VFREE(m);
		fclose(input);
		return(NULL);
	}
	...
	for(i = 0; i < samples; i++) {          // read the sample headers
		...
		m->slength[i] = (fgetc(input) * 256L+ fgetc(input))*2;
	...
	j = fread(m->ptable,1,128,input);       // read pattern table
	if(j != 128) {
		sprintf(errstring,"Error Reading Pattern Map");
		VFREE(m);
		fclose(input);
		return(NULL);
	}
	...
	for(i = 0; i <samples; i++) {           // Read in the sample data
		...
		if(m->slength[i] <0 || m->slength[i] > 0x0000ffff) {
			for(j = 0; j <= m->maxpattern; j++) VFREE(m->pattern_data[j]);
			for(j = 0; j < i; j++) VFREE(m->sdata[j]);
			sprintf(errstring,"Weird Sample length");
			VFREE(m);
			fclose(input);
			return(NULL);
		}
