...
//CHAR s3mfilename[255] = {"armani.s3m"};
...
/*void punish(void)
{
	...
	printf("Entered Punish.\n");
																					// Set up the sound blaster.
  if(!SBSetUp()) {
		printf("SB_Setup returned this error: %s \n",errname[sberr]);
		return;
  }
	printf("Set up Spund Blaster\n");

	GoVarmint();                            // Start up the sound Kernel.
	printf("Varmint Started\n");

	while(r != 'q') {
		PlaySound(sound1,v_plain);      // Play a sound effect.
		printf("Played a sound\n");
		r = getch();
	}
	printf("Done with sounds\n");

  DropDeadVarmint();                      // Stop the SB interrupt.
	printf("Varmint stopped\n");
...
/*	for(i = 0; i < 100 ; i++) {
		if(mys3m->sdata[i]) printf("%d] (%ld)(%ld,%ld,%d) %s\n",i,mys3m->slength[i],
				mys3m->loopbegin[i],mys3m->loopend[i],(int)(mys3m->sflag[i]),
				mys3m->sample_name[i]);
	}
	getch(); */
...
/*		inits3m();
		...
		while(j && s3m_on) {
			j++;
			if(!(j%100)) {                     // check keyboard once in a while
				if(kbhit()) j =0;
			}
			s3m_bytespertick = 3;
			s3mplayer();
			gotoxy(10,10);
			printf("%ld,%d   ",debugnum,mys3m->orders - s3m_order);
			if(debugnum == 1547) {
				debugnum = debugnum;
			}
		}
...
void vsyncdemo(void)
{
	...
	vcprintf("when you play sounds.  (This does not work well with MOD's\r\n");
	vcprintf("on slow computers like 386's.)\r\n\n");
	textcolor(LIGHTGRAY);
