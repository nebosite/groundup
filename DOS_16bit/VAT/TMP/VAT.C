...
SHORT       disabled = 0;
WORD        DSP_overhead = 0;
FASTINT			bytes_per_synccheck = 10;
...
BYTE        sync_on = FALSE;
																	...
																	// startup problems
CHAR        vatdebugstring[5000] = {"*** DEBUGGING STRING ***\n"};
CHAR        vatapstr[255];
SHORT				vinit;
...
static SHORT testint(void)
{
	...
	outportb(0x21,0xff);                      // Turn off all interrupts
	outportb(0xa1,0xff);
...
WORD SetSampleRate(WORD rate)
{
	...
	if(rate>22222L && dsp_vers < 0x200) rate = (WORD)22222L;
...
void TimeVSync(void)
{
	...
	if(l < 100) l = 17000;         // Sanity measure

																 // Figure how many vclock ticks will occur
																 // During a retrace.
	vsync_toolong = (WORD)((LONG)l * sample_rate / 1193000L / dma_bufferlen) + 2;
}
...
/**************************************************************************
	...
	DESCRIPTION:  Special interrupt disable function for VAT
