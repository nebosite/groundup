		...
		for(i=0;i<channels;i++){             //loop through ALL channels...
			if(chan[i].volume && *chan[i].pos < chan[i].end) {
				...
				for(j=0;j<dma_bufferlen;j++){
					*(mix_buffer+j) += (*(chan[i].sdata + (ARRAYTYPE)*chan[i].pos)*vol)>>6;
					chan[i].position+=chan[i].pinc;
					if(*chan[i].pos>=chan[i].end) {
						if(chan[i].sflag&0x1) {      //If this inst loops loop it...
//            chan[i].position=chan[i].offset*256L;  <---------replace this

							*chan[i].pos -= (chan[i].rlength-chan[i].offset);
//							chan[i].position=(chan[i].offset+                //with this
//																(*chan[i].pos%chan[i].rlength))*256L;
							chan[i].counter=0;
...
{                                      //local variables
  ...
  BYTE infox,infoy;
	...
	//		chan[k].period=(LONG)(133808.0*(s3m_period[note]/(float)(1<<octave))/s3m_data->c2spd[chan[k].sample_number]);
	//    chan[k].period=133808*(s3m_period[note]>>octave)/s3m_data->c2spd[chan[k].sample_number];
	//    if(chan[k].period>100000)exit(1);
    ...
    switch(command) {
			...
			case 7:                              //Tone port. still uses floating point
				...
				if(note!=255) chan[k].period2=
					(LONG)(10L*((13381L*s3m_period[note])/(1<<octave))/
					 s3m_data->c2spd[chan[k].sample_number]);
//				if(note!=255) chan[k].period2=(LONG)(133808.0*(s3m_period[note]/(float)(1<<octave))/s3m_data->c2spd[chan[k].sample_number]);
		//    chan[k].period2=133808*(s3m_period[note]>>octave)/s3m_data->c2spd[chan[k].sample_number];
				break;
			case 8:                              //Vibrato
				...
				// ECODE
				if(chan[k].vibdepth <0 ||  chan[k].vibdepth > 15) {
					debugnum++;
				}
				break;
...
S3M VFAR *LoadS3M(CHAR *filename,CHAR *errstring)
{
  ...
  for(i=0;i<s->insnum;i++){            // read in the raw data for each inst.
    ...
    if(temp2>66) {                     // if pattern not all 0's read it in
      ...
      if(!s->pattern_data[i]){
				for(j=0;j<i;j++) VFREE(s->pattern_data[j]);
        sprintf(errstring,"Out of Memory");
				VFREE(s);
				fclose(input);
				return(NULL);
