...
#define DOSYNC1    if(sync_on && !(j % bytes_per_synccheck) && inportb(0x3da)&0x08) vsyncclock = 1000
#define DOSYNC2    if(sync_on && inportb(0x3da)&0x08) vsyncclock = 1000
...
void dodiv(void)
{
	...
	for(i = 0; i < 4; i++) {                // Loop through all four channels
		...
		if(samp && e1 != 3 && period) {                 // set a new sample?
			...
			if(chan[i].rlength) chan[i].end = chan[i].rlength;
