...
//
// Computer.cpp - computer intelligence code.
...
void advance(void);
void thinkinit(bool justchecking = false);
...
int ispeedgrid[10][2] = {								// Intelligence speed Grid
	{20,1},
	{12,1},
	{8,1},
	{6,1},
...
**************************************************************************/
void thinkinit(bool justchecking)
{
...
void thinkharder()
{
	...
	if(p->computer_smarts > 3 &&  t->movespecial == SPECIALBASE +6) {
		if(p->enemy && (countblocks(p->enemy->spot) - countblocks(p->spot) > 0)) {
...
/*void drwgrid(void)
{
	...
	g_lpdvdrawobject->DrawFillRectangle(0,p->gridx,yoff,p->gridx + scale * grid_width,maxy);
	s = 0;
	for(y = 0; y < grid_height; y++) {
		for(x = 0;x < grid_width; x++) {
			if(*(t->grid+s))
				g_lpdvdrawobject->DrawFillRectangle(YELLOW666,
					x*scale+p->gridx,
					y*scale+yoff,
...
void cogitate(PLAYER *plyr)
{
	...
	if(!plyr) {
		g_lpdvdrawobject->Print(13,10,10,"Yikes!");
		return;
	}
...
double trymove(void)
{
	...
	if(rot < 0 || rot > 3 || trans < -2 || trans > 10) {
		//g_lpdvdrawobject->Print(14,10,10,"THINKING ERROR: R%d T%d",rot,trans);
		return MOVEERR;
	}
	...
	y--;
	score = csetpiece(x,y,&hold,grid);
//E	score = 1000 - csetpiece(x,y,&hold,grid);
	...
	*/
	...
	// TODO: try switching these lines, and adding x_row to score
...
void copygrid(BYTE g1[], BYTE g2[])
{
	memcpy(g1,g2,grid_width*grid_height );
}
...
//E		if(empties) empties = 10;

		emptyscore+= empties;
	}

	return emptyscore;
//E	return(emptyscore + (grid_height-topmost)*grid_width);
//	return emptyscore;
}
...
int csetpiece(int px, int py,PIECE *p,BYTE g[])
{
	...
	coords = p->bloc[p->rotation];
								...
								// Check the contour score
	for(i = 0; i < 4; i++) {
		x = px + coords[i*2] ;
		y = py + coords[i*2+1];

		if(x == 0) contourscore ++;
		else if(g[(x-1) + y*grid_width]) contourscore ++;
		if(x >= grid_width-1) contourscore++;
		else if(g[(x+1) + y*grid_width]) contourscore ++;

		if(y >= grid_height-1 ) contourscore++;
		else if(g[x + (y+1)*grid_width]) contourscore ++;
...
/*===========================================================================
	...
	DESCRIPTION:	returns the "score" fo the current move.  I use this function 
					to test my computer logic.

	  REVISIONS:
			12/28/99		Eric Jorgensen		Initial Version
