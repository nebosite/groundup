...
double speedup = 0.95;
PLAYER far *person[MAXPLAYERS];
char *names[] = {"Fred","Wilma","Betty","Barney","Pebbles","BamBam","Dino",
									"Mr Slate","George","Jane","Elroy","Judy","Max"};
BYTE charbuffer[256];
...
int piecedata[8][4][8] = {
 ...
 {{1,1,2,1,1,2,2,2},{1,1,2,1,1,2,2,2},       // Piece 7  (Box)
	{1,1,2,1,1,2,2,2},{1,1,2,1,1,2,2,2}}
																		...
																		// 80 background tiles
																		// 200 dizzolve blocks
RasterBlock far *shape[256],*dummy[2];
BGROUND far *bgrnd[50];
...
void main(void)
{
	...
	setup();                             // Get graphics and that going


	readplayerconfig();                  // Read player configuration

                                       // Start up sound engine
	...
	mainmenu();                          // run the game


	if(soundtype == SBSOUND) {           // Clean up sound stuff
		MilliDelay(200);
		DropDeadVarmint();
		MilliDelay(200);
		SBCleanUp();
	...
	sound(10000);                        // Clean up possible speaker probs.
	nosound();

	restext();                           // bye bye!

	printf("Thanks for playing Varmint's EITtris!\n");
	gotoxy(1,10);
	if(random(22) == 1) printf("Remember:  EIT is not just a word, it's an acronym.\n\n");
	else printf("Remember:  EIT is not just a word, it's a way of life.\n\n");
	generatescores();
}
