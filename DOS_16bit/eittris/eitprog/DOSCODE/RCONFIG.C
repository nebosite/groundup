...
void readconfig(void)
{
  ...
  if(input) {
    while(fgets(string,999,input)) {    // Read each line...
        ...
        i++;
        ...
        else if(!strcmp(id,"NOSOUND")) {
        	soundtype = NOSOUND;
        }
        else if(!strcmp(id,"PCSOUND")) {
          soundtype = PCSOUND;
        }
				else if(!strcmp(id,"SBSOUND")) {
          soundtype = SBSOUND;
        }
				else if(!strcmp(id,"SBVERSIONFIX")) {
					debug_lowdsp = 0x200;
        }
          ...
          snapshotmode = TRUE;
        	...
        	textcolor(RED);
        	cprintf("ERROR- Unknown option in config file: %s\n\r",id);
        }
      }
    }
  	fclose(input);
