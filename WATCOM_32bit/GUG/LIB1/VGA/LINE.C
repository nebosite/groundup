...
void GUG_h_line(char **ScreenPtr, int XAdvance, int RunLength,
                int Color);
void GUG_v_line(char **ScreenPtr, int XAdvance, int RunLength,
                int Color);

// Draws a line between the specified endpoints in color Color.
void GUGLine(int XStart, int YStart, int XEnd, int YEnd, int Color)
{
...
// Determine whether the line is X or Y major, and handle accordingly
   if (XDelta >= YDelta)
   {
      ...
      GUG_h_line(&ScreenPtr, XAdvance, InitialPixelCount, Color);
      // Draw all full runs 
      for (i=0; i<(YDelta-1); i++)
      {
         ...
         GUG_h_line(&ScreenPtr, XAdvance, RunLength, Color);
      }
      // Draw the final run of pixels
      GUG_h_line(&ScreenPtr, XAdvance, FinalPixelCount, Color);
      return;
   }
   else
   {
      ...
      // Draw the first, partial run of pixels
      GUG_v_line(&ScreenPtr, XAdvance, InitialPixelCount, Color);
      ...
      for (i=0; i<(XDelta-1); i++)
      {
         ...
         // Draw this scan line's run
         GUG_v_line(&ScreenPtr, XAdvance, RunLength, Color);
      }
      // Draw the final run of pixels
      GUG_v_line(&ScreenPtr, XAdvance, FinalPixelCount, Color);
      return;
   }
}
...
void GUG_h_line(char **ScreenPtr, int XAdvance,
...
// the first pixel of the next run.
void GUG_v_line(char **ScreenPtr, int XAdvance,
                int RunLength, int Color)
{
   int i;
   char *WorkingScreenPtr = *ScreenPtr;
