...
#include <malloc.h>
#include <stdarg.h>
#include <conio.h> // Remove me later
#include <graph.h> // Remove me later
#include "vat.h"
#include "internal.h"
#include "compat.h"

#define DEBUGME fprintf
...
void zerocheck(char *string, int testme)
{
	return;
	if(!testme) {
		fprintf(stderr,"  DIVIDE ERROR at: %s       ",string);
		getch();
	}
		...
		if(s3m_currentbyte <= 0 ) {
			if(s3m_currenttick >= s3m_ticksperdivision*(s3m_patterndelay+1)) {
				...
				if((s3m_pattern == s3m_pstop) || s3m_hardstop) {
					...
					while((int)(s3m_data->order[s3m_order]) == 0xfe &&      // Skip pattern?
								s3m_order < s3m_data->orders) {
						s3m_order++;
					}
...
//gotoxy(1,1);
//DEBUGME(stderr,"Ch: %d (%d)   ",k,chan[k].pinc);
		...
		if(instrument) {
																				 // Make sure instrument is valid
			if(instrument > 100) instrument = 0;
			if(!s3m_data->sdata[instrument-1]) instrument = 1;
			while(!s3m_data->sdata[instrument-1]) instrument++;
			...
			if(volume==255) chan[k].volume=s3m_data->volume[instrument-1];
		}

																				 // We need to calculate a new
																				 // period IF:
																				 //   A) a period has not yet been
																				 //      calculated (period == 0)
																				 //
																				 //							-or-
																				 //
																				 //	  B) If a note is specified and
																				 //      we aren't slifing to a new
																				 //      note.

		if((note!=255)&&(command!=7)){

																				 // Make sure note is valid
			octave += note/12;
			note = note % 12;

//DEBUGME(stderr,"(%d,%d)  ",chan[k].sample_number,instrument);
			zerocheck("DD0a",s3m_data->c2spd[chan[k].sample_number]);
			chan[k].period=
				(LONG)(10L*((13381L*s3m_period[note])/(1<<octave))/
				s3m_data->c2spd[chan[k].sample_number]);
			zerocheck("DD0b",chan[k].period);
			rate=(DWORD)(14317056L/chan[k].period);
			zerocheck("DD0c",internal_sample_rate);
			chan[k].pinc=(((DWORD)rate*256L)/internal_sample_rate);
			chan[k].position=0;
			chan[k].counter=0;
		}
//DEBUGME(stderr,"(%d) N:%d S: %d  ",chan[k].pinc,note,s3m_data->c2spd[chan[k].sample_number]);

//gotoxy(35,1);
//DEBUGME(stderr,"(C:%d,%2x)    ",command,info);
		switch(command) {
			case 1:                              //Set Speed
        ...
        if(infox==15) {
					chan[k].period+=infoy*4;
					chan[k].fineperiod=infoy*4;
					zerocheck("D5a",chan[k].period);
					rate=(DWORD)(14317056L/chan[k].period);
					zerocheck("D5b",internal_sample_rate);
					chan[k].pinc=(((DWORD)rate*256L)/internal_sample_rate);
					chan[k].porta=0;
				...
				if(infox==14) {
					...
					zerocheck("D5c",chan[k].period);
					rate=(DWORD)(14317056L/chan[k].period);
					zerocheck("D5d",internal_sample_rate);
					chan[k].pinc=(((DWORD)rate*256L)/internal_sample_rate);
					chan[k].porta=0;
					chan[k].s3m_fine=v_true;
					break;
				...
				if(chan[k].s3m_fine){
					...
					zerocheck("D5e",chan[k].period);
					rate=(DWORD)(14317056L/chan[k].period);
					zerocheck("D5f",internal_sample_rate);
					chan[k].pinc=(((DWORD)rate*256L)/internal_sample_rate);
					chan[k].porta=0;
				}
				break;
			case 6:                              //slide note up fine and extra fine too.
				if(infox==15) {
					chan[k].period-=infoy*4;
					chan[k].fineperiod=infoy*4;
					zerocheck("D6a",chan[k].period);
					rate=(DWORD)(14317056L/chan[k].period);
					zerocheck("D6b",internal_sample_rate);
					chan[k].pinc=(((DWORD)rate*256L)/internal_sample_rate);
					chan[k].porta=0;
					chan[k].s3m_fine=v_true;
					break;
				...
				if(infox==14) {
					chan[k].period-=infoy;
					chan[k].fineperiod=infoy;
					zerocheck("D6c",chan[k].period);
					rate=(DWORD)(14317056L/chan[k].period);
					zerocheck("D6d",internal_sample_rate);
					chan[k].pinc=(((DWORD)rate*256L)/internal_sample_rate);
					chan[k].porta=0;
					chan[k].s3m_fine=v_true;
					break;
				...
				if(chan[k].s3m_fine){
					chan[k].period-=chan[k].fineperiod;
					zerocheck("D6e",chan[k].period);
					rate=(DWORD)(14317056L/chan[k].period);
					zerocheck("D6f",internal_sample_rate);
					chan[k].pinc=(((DWORD)rate*256L)/internal_sample_rate);
					chan[k].porta=0;
				}
				break;
			case 7:                             // Tone portamento
				...
				if(info) chan[k].porta=info*4;
				zerocheck("D7",s3m_data->c2spd[chan[k].sample_number]);
				if(note!=255) chan[k].period2=
					(LONG)(10L*((13381L*s3m_period[note])/(1<<octave))/
					 s3m_data->c2spd[chan[k].sample_number]);
				if(chan[k].period < 1) chan[k].period = chan[k].period2;
				break;
			case 8:                              //Vibrato
...
//        s3m_bytespertick=(WORD)((((LONG)internal_sample_rate*s3m_bytespertick)/11000L))<<vat_stereo;
				...
				zerocheck("D20",info);
				if(info) // protect against divide by zeros
...
//				if(infox || infoy)s3m_vibratocount=0;
		...
		s3m_pattern+=5;
//if(chan[k].pinc > 19000) {
//DEBUGME(stderr,"BIG PINC!! (%d)   ",chan[k].pinc);
//getch();
//}
//if(chan[k].period < 0 ) {
//DEBUGME(stderr,"Period Error (%d) ",chan[k].period);
//getch();
//}

	}
//gotoxy(1,1);
//DEBUGME(stderr,"POST CHANNELS... ");
...
WORD scalcvibrato(WORD inc,WORD period,WORD amplitude)
{
	...
	newinc = newinc + (LONG)((LONG)vsin[(WORD)theta] * amplitude * 2);
...
//gotoxy(1,2);
//DEBUGME(stderr,"Tk: %d,%d    ",i,chan[i].effect);
		switch(chan[i].effect) {
			...
			case 5:                            //Slide note down
				...
				zerocheck("T5a",chan[i].period);
				rate=(DWORD)14317056L/(chan[i].period);
				zerocheck("T5b",internal_sample_rate);
				chan[i].pinc=(((DWORD)rate*256L)/internal_sample_rate);
				break;
			case 6:                            //Slide note up
				chan[i].period-=chan[i].porta;
				zerocheck("T6a",chan[i].period);
				rate=(DWORD)14317056L/chan[i].period;
				zerocheck("T6b",internal_sample_rate);
				chan[i].pinc=(((DWORD)rate*256L)/internal_sample_rate);
				break;
			case 7:                            //tone portamento
...
//if(chan[i].period < 0 ) {
//gotoxy(1,1);
//DEBUGME(stderr,"Period Error in sdotick() (%d) ",chan[i].period);
//getch();
//}
				zerocheck("T7a",chan[i].period);
				if(chan[i].period<1)chan[i].period=1;
				rate=(DWORD)14317056L/chan[i].period;
				zerocheck("T7b",internal_sample_rate);
				chan[i].pinc=(((LONG)rate*256L)/internal_sample_rate);
				break;
			case 8:                            //Vibrato
				zerocheck("T8a",chan[i].period);
				...
				zerocheck("T8b",internal_sample_rate);
				chan[i].pinc=(((LONG)rate*256L)/internal_sample_rate);
				break;
			case 11:                           //vibrato+volumeslide
				chan[i].volume+=chan[i].vslide;
				...
				zerocheck("T11a",chan[i].period);
				if(!chan[i].period)chan[i].period=1;
				period=scalcvibrato(chan[i].period,chan[i].vibperiod,chan[i].vibdepth);
				rate=(DWORD)(14317056L/period);
				zerocheck("T11b",internal_sample_rate);
				chan[i].pinc=(((LONG)rate*256L)/internal_sample_rate);
				break;
			case 12:                           //port+volumeslide
				...
				zerocheck("T12a",chan[i].period);
				if(chan[i].period<1)chan[i].period=1;
				rate=(DWORD)(14317056L/chan[i].period);
				zerocheck("T12b",internal_sample_rate);
				chan[i].pinc=(((LONG)rate*256L)/internal_sample_rate);
				break;
			case 17:                           //retrig + volumeslide
			...
			case 19:                           //special
				...
				break;
			case 21:                           //fine vibrato
				zerocheck("T21a",chan[i].period);
				if(!chan[i].period)chan[i].period=1;
				period=scalcvibrato(chan[i].period,chan[i].vibperiod,chan[i].vibdepth);
				rate=(DWORD)(14317056L/period);
				zerocheck("T21b",internal_sample_rate);
				chan[i].pinc=(((LONG)rate*256L)/internal_sample_rate);
				break;
			default:
				break;
		...
		chan[i].volume *= s3m_channelselect[i];  // THis allows the user to mask
																						 // channels
//if(chan[i].period < 0 ) {
//DEBUGME(stderr,"Period Error in Tick (%d) ",chan[i].period);
//getch();
//}
//if(chan[i].pinc > 19000) {
//DEBUGME(stderr,"BIG PINC in tick (%d,%d,%d,%d)  ",
//chan[i].pinc,chan[i].period,chan[i].period2,chan[i].porta);
//getch();
//}
	}
//DEBUGME(stderr,"END TICK   ",i,chan[i].effect);
}
...
S3M  *LoadS3M(CHAR *s3mfile,CHAR *errstring)
{
	...
	S3M *s;
	BYTE ftype,control;
//	BYTE *db[20]; // Remove me after you are doen debugging

	//DEBUGME(stderr,"ENTRY"); // Debugging Code
	...
	//DEBUGME(stderr,"0"); // Debugging Code
	...
	s->chainfunc = NULL;
	s->repeatmusic = 0;
	//DEBUGME(stderr,"1 S: %d\n",(int)s); // Debugging Code
																			 // initialize the s3m struct
	//DEBUGME(stderr,"2"); // Debugging Code
	for(i = 0; i < 100; i++) {
		s->sample_name[i][0] = 0;
		s->slength[i] = 0;
		s->sflag[i] = 0;
	...
	//DEBUGME(stderr,"3"); // Debugging Code
	...
	//DEBUGME(stderr,"4"); // Debugging Code
	...
	//DEBUGME(stderr,"5"); // Debugging Code
...
//-DEBUGME(stdout,"TITLE:  \"%s\"  :::\n",s->title);
//-DEBUGME(stdout,"  ordnum: %d\n",ordnum);
//-DEBUGME(stdout,"  insnum: %d\n",s->insnum);
//-DEBUGME(stdout,"  patnum: %d\n",s->patnum);
//-DEBUGME(stdout,"  initspeed: %d\n",s->initspeed);
//-DEBUGME(stdout,"  inittempo: %d\n",s->inittempo);
	s->channelnum=0;                     // get number of channels in the song
	for(i=0;i<32;i++){
		temp=fgetc(input);
		if (temp!=255) s->channelnum++;
	}
//-DEBUGME(stdout,"  channelnum: %d\n",s->channelnum);
																			 // Read in the orders
	for(i=0;i<ordnum;i++){

		temp=fgetc(input);
		if (temp!=255) s->order[i]=temp;   // 255 means this is the last order
		else break;
	}
	s->orders=i;                         // reset order number for real number
//-DEBUGME(stdout,"  new orders: %d\n",s->orders);
																			 // of orders.
	//DEBUGME(stderr,"6"); // Debugging Code

//-DEBUGME(stdout,"Reading instrument offsets...........\n");
	fseek(input,0x60+ordnum,SEEK_SET);
	for(i=0;i<s->insnum;i++){            // read the instrument info offsets
		fread((char *)&instpoint[i],2,1,input);
//-DEBUGME(stdout,"  %d\n",instpoint[i]);
	}
//-DEBUGME(stdout,"Reading pattern offsets...........\n");
...
//-DEBUGME(stdout,"  %d\n",pattpoint[i]);
	...
	//DEBUGME(stderr,"7"); // Debugging Code

//-DEBUGME(stdout,"Reading instument info>>>>>>>>>>>>>>>>>>>>.\n");
	for(i=0;i<s->insnum;i++){            // start reading instrument info
		fseek(input,instpoint[i]*16,SEEK_SET); // move to offset.
//-DEBUGME(stdout,"  [%d] @%d: ",i,instpoint[i]*16);
		temp2=fgetc(input);
//-DEBUGME(stdout,"%d ",temp2);
...
//-DEBUGME(stdout,"\"%s\" \n",s->sample_name[i]);
//-DEBUGME(stdout,"    parapointer: %d \n",paras[i]);
//-DEBUGME(stdout,"    slength: %d \n",s->slength[i]);
//-DEBUGME(stdout,"    loopbegin: %d \n",s->loopbegin[i]);
//-DEBUGME(stdout,"    loopend: %d \n",s->loopend[i]);
//-DEBUGME(stdout,"    volume: %d",s->volume[i]);
		}
//-DEBUGME(stdout,"\n",s->volume[i]);
	}

	//DEBUGME(stderr,"8"); // Debugging Code

//-DEBUGME(stdout,"Reading instument Raw Data------------------------.\n");
	for(i=0;i<s->insnum;i++){            // read in the raw data for each inst.
//-DEBUGME(stdout,"  [%d] @%d: \n",i,paras[i]/16);
		if(paras[i]){
//-DEBUGME(stdout,"    Length: %d ",s->slength[i]);
...
//DEBUGME(stderr,"Alloc: %d\n",(int)(s->slength[i]+1)); // Debugging Code
	...
	//DEBUGME(stderr,"9"); // Debugging Code

	s->insnum=temp;                      // update to real number of insts.
//-DEBUGME(stdout,"Real insnum: %d\n",s->insnum);
	temp=0;
	vbyte = s->channelnum * 5;           // Calculate number of bytes
																		 ...
																		 // Pre-allocate pattern Data
//	for(i = 12; i < 17; i++) {
//-DEBUGME(stdout,"Allocating memory::::::::::\n");
	for(i=0;i<s->patnum;i++){
		//DEBUGME(stderr,"aa:%d \n",2880); // Debugging Code
		s->pattern_data[i]=(BYTE  *)VMALLOC(vbyte*64); // see if theres room
//-DEBUGME(stdout,"  dp[%d]: %d\n",i,s->pattern_data[i]);
	}


//-DEBUGME(stdout,"Reading Pattern Info***********************\n");
	for(i=0;i<s->patnum;i++){            // start reading pattern info
		//DEBUGME(stderr,"\nP:%d ",(int)(pattpoint[i]*16)); // Debugging Code
		fseek(input,pattpoint[i]*16,SEEK_SET);
		fread((char *)&temp2,2,1,input);
//-DEBUGME(stdout,"  [%d]: @%d: %d\n",i,pattpoint[i]*16,temp2);
		//DEBUGME(stderr,"  C:%d ",(int)temp2); // Debugging Code
		if(temp2>66 && temp2<10240) {      // if pattern not all 0's read it in
//-DEBUGME(stdout,"    VALID\n");
			temp++;
			//DEBUGME(stderr,"  M%dL:%d ",i,(int)(vbyte*64)); // Debugging Code
			//DEBUGME(stderr,"  p:%d ",(int)(s)); // Debugging Code
			if(!s->pattern_data[i]) s->pattern_data[i]=(BYTE  *)VMALLOC(vbyte*64); // see if theres room
			//DEBUGME(stderr,"  PML:%d ",(int)(vbyte*64)); // Debugging Code
			if(!s->pattern_data[i]){
				FreeS3M(s);
				sprintf(errstring,"Out of Memory");
				fclose(input);
			...
			//DEBUGME(stderr,"init "); // Debugging Code
			...
			//DEBUGME(stderr,"ctrl "); // Debugging Code
	...
	fclose(input);
	//DEBUGME(stderr,"10"); // Debugging Code

	s->patnum=temp;                      //This assigns the real number of patts
	//DEBUGME(stderr,"11"); // Debugging Code
	return(s);
}                                      // that's all folks!!
