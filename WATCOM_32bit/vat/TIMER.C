...
/**************************************************************************
  ...
  DESCRIPTION: Turns off time and reports clicks elapsed.  Note that this
               ...
               milliseconds.  If you want to time longer stuff, I suggest
...
WORD TimerOff(void)
{
  ...
  timer_hold = (WORD)VINPORTB(0x42);
  timer_hold += (WORD)VINPORTB(0x42) * 256;
...
/**************************************************************************
  ...
  DESCRIPTION: loop for a standard measure function.
...
void _saveregs MilliDelay(WORD delay)
{
  ...
  for(i=0;i<delay;i++) mdelay((DWORD)mue999); // mdelay(mue999) = 1 millisec
}
...
**************************************************************************/
void ackint(void);                 // acknowledge interrupt
#pragma aux ackint=\
  "mov al,20"     \
  "out 20,al"     \
  modify [al];
...
/**************************************************************************
  void InstallTimer0(WORD period,void  (*func)())
...
**************************************************************************/
// program timer 0 for use in InstallTimer0 function
// programs timer 0  with modus 3 and counter of period

void progt0(WORD period);
#pragma aux progt0 =\
  "mov al,0x36 "\
  "out 0x43,al "\
  "mov ax,cx   "\
...
**************************************************************************/
// this function is for use in RemoveTimer0 function.
// it is the inline code that sets the timer back to the way is was.

void setbackt0(void);
#pragma aux setbackt0 = \
  "mov al,0x36"\
  "out 0x43,al"\
  "mov al,0   "\
  "out 0x40,al"\
